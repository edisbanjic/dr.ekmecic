...
import { postaviLozinku, type LozinkaState } from "@/app/admin/auth-actions";
...
export default function PostaviLozinkuPage() {
  const [state, formAction, pending] = useActionState<LozinkaState, FormData>(postaviLozinku, {});
    ...
    >
      ...
      >
        ...
        <label>
          ...
          <input required minLength={8} type="password" name="lozinka" autoComplete="new-password" style={poljeStyle} />
        </label>
        <label>
          <span style={{ display: "block", fontWeight: 800, fontSize: "13.5px", marginBottom: "6px" }}>
            Potvrdite lozinku
          </span>
          <input required minLength={8} type="password" name="potvrda" autoComplete="new-password" style={poljeStyle} />
        </label>
        {state.error && (
