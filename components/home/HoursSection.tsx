...
export default function HoursSection() {
  ...
  return (
    <>
      ...
      <section id="hours" style={{background:"#F5F0E8",padding:"clamp(60px,9vw,110px) 0",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",maxWidth:"1200px",margin:"0 auto",padding:"0 clamp(18px,4vw,32px)",textAlign:"center"}}>
          <div data-reveal="">
            ...
            <h2 style={{margin:"16px 0 0",fontFamily:"var(--font-fredoka)",fontWeight:"700",fontSize:"clamp(38px,5.6vw,62px)",lineHeight:"1.02"}}>{t.title}</h2>
          </div>
          <div data-reveal="" data-delay="120" style={{position:"relative",maxWidth:"860px",margin:"clamp(32px,5vw,48px) auto 0",background:"#3D4142",color:"#F5F0E8",borderRadius:"44px",padding:"clamp(28px,5vw,52px)",transform:"rotate(-.6deg)",boxShadow:"0 36px 64px -32px rgba(61,65,66,.55)",textAlign:"left"}}>
            ...
            <div style={{display:"grid",gap:"8px"}}>
              <div style={{display:"flex",alignItems:"baseline",gap:"14px",padding:"16px 18px",borderRadius:"18px"}}>
                <div style={{fontFamily:"var(--font-fredoka)",fontWeight:"600",fontSize:"clamp(17px,2.4vw,22px)"}}>{t.monWed}</div>
                <div style={{flex:"1",borderBottom:"3px dotted rgba(245,240,232,.3)",transform:"translateY(-5px)"}}></div>
                <div style={{textAlign:"right"}}><div style={{fontWeight:"800",fontSize:"clamp(16px,2.3vw,20px)"}}>08:00 – 16:00</div><div style={{fontSize:"13px",opacity:".6"}}>{t.pauseShort}</div></div>
              </div>
              <div style={{display:"flex",alignItems:"baseline",gap:"14px",padding:"16px 18px",borderRadius:"18px",background:"#7EAEE8",color:"#243038",boxShadow:"0 14px 28px -16px rgba(126,174,232,.9)"}}>
                <div style={{fontFamily:"var(--font-fredoka)",fontWeight:"600",fontSize:"clamp(17px,2.4vw,22px)",display:"flex",alignItems:"center",gap:"10px",flexWrap:"wrap"}}>{t.thursday} <span style={{background:"#FFFFFF",borderRadius:"999px",padding:"3px 11px",fontSize:"11.5px",letterSpacing:".1em",fontWeight:"600"}}>{t.thursdayTag}</span></div>
                <div style={{flex:"1",borderBottom:"3px dotted rgba(36,48,56,.35)",transform:"translateY(-5px)"}}></div>
                <div style={{textAlign:"right"}}><div style={{fontWeight:"800",fontSize:"clamp(16px,2.3vw,20px)"}}>10:00 – 18:00</div><div style={{fontSize:"13px",opacity:".7"}}>{t.pauseThursday}</div></div>
              </div>
              <div style={{display:"flex",alignItems:"baseline",gap:"14px",padding:"16px 18px",borderRadius:"18px"}}>
                <div style={{fontFamily:"var(--font-fredoka)",fontWeight:"600",fontSize:"clamp(17px,2.4vw,22px)"}}>{t.friday}</div>
                <div style={{flex:"1",borderBottom:"3px dotted rgba(245,240,232,.3)",transform:"translateY(-5px)"}}></div>
                <div style={{textAlign:"right"}}><div style={{fontWeight:"800",fontSize:"clamp(16px,2.3vw,20px)"}}>08:00 – 16:00</div><div style={{fontSize:"13px",opacity:".6"}}>{t.pauseShort}</div></div>
              </div>
              <div style={{display:"flex",alignItems:"baseline",gap:"14px",padding:"16px 18px",borderRadius:"18px"}}>
                <div style={{fontFamily:"var(--font-fredoka)",fontWeight:"600",fontSize:"clamp(17px,2.4vw,22px)"}}>{t.weekend}</div>
                <div style={{flex:"1",borderBottom:"3px dotted rgba(245,240,232,.3)",transform:"translateY(-5px)"}}></div>
                <div style={{textAlign:"right"}}><div style={{fontFamily:"var(--font-shantell)",fontWeight:"600",fontSize:"clamp(16px,2.3vw,19px)",color:"#F4A08A"}}>{t.closed}</div><div style={{fontSize:"13px",opacity:".6"}}>{t.closedNote}</div></div>
              </div>
            </div>
            <div style={{marginTop:"18px",paddingTop:"16px",borderTop:"1px solid rgba(245,240,232,.15)",fontSize:"14px",opacity:".75",display:"flex",gap:"8px",alignItems:"baseline"}}><span style={{color:"#7EAEE8"}}>✦</span>{t.tip}</div>
          </div>
        </div>
      </section>
    </>
