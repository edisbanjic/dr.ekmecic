...
export default function BookingForm() {
  ...
  const [podaci, setPodaci] = useState<Podaci>({ ime: "", telefon: "", usluga: USLUGE[0], napomena: "" });
  ...
  const [sent, setSent] = useState<{ datum: string; vrijeme: string } | "predlozi" | null>(null);
  ...
  const skrolNaVrh = () => vrh.current?.scrollIntoView({ behavior: "smooth", block: "start" });

  const naKorak2 = () => {
    if (!podaci.ime.trim() || !podaci.telefon.trim()) {
  ...
  const posalji = (predlozi: boolean) => {
    if (!predlozi && (!datum || !vrijeme)) {
      setError("Odaberite datum i vrijeme, ili kliknite „Predložite mi termin“.");
      return;
    }
  ...
  if (sent) {
    const poruka =
      sent === "predlozi" ? (
        <>Hvala na povjerenju! Nazvat ćemo vas u najkraćem roku i zajedno pronaći termin koji vam odgovara.</>
      ) : (
        <>
          Rezervisali ste {parseDatum(sent.datum).getDate()}.{" "}
          {MJESECI[parseDatum(sent.datum).getMonth()].toLowerCase()} u {sent.vrijeme}. Nazvat ćemo
          vas u najkraćem roku da potvrdimo termin.
        </>
      );
    return (
      <div
  ...
  return (
    <div ref={vrh} style={{ display: "grid", gap: "18px", scrollMarginTop: "110px" }}>
      {/* indikator koraka */}
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: "16px" }}>
            ...
            <label style={{ display: "block", gridColumn: "1 / -1" }}>
              ...
              <select
                value={podaci.usluga}
                onChange={(e) => setPodaci({ ...podaci, usluga: e.target.value })}
                style={{ ...fieldStyle, appearance: "none" }}
              >
                {USLUGE.map((u) => (
            ...
            <label style={{ display: "block", gridColumn: "1 / -1" }}>
              ...
              <textarea
                ...
                onChange={(e) => setPodaci({ ...podaci, napomena: e.target.value })}
                rows={2}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "16px", flexWrap: "wrap" }}>
            <button type="button" onClick={naKorak2} className="hv-cta" style={dugmeStyle}>
